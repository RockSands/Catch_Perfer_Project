...
public class ItemRecommendCSV2DBBuild {
	...
	private TransMeta createFirstTrans() throws KettleException {
		...
		ci.setFilename(recommendPropeties.getItemMixRecommendFileName());
	...
	private TransMeta createSecondTrans() throws KettleException {
		String uuid = UUID.randomUUID().toString().replace("-", "");
		TransMeta transMeta = null;
		transMeta = new TransMeta();
		transMeta.setName("CTIS-" + uuid);
		...
		ci.setDelimiter(",");
		ci.setEncoding("UTF-8");
		ci.setFilename(recommendPropeties.getItemGrRecommendFileName());
		ci.setIncludingFilename(false);
		ci.setAddResultFile(false);
		ci.setBufferSize("50000");
		TextFileInputField field_0 = new TextFileInputField();
		...
		TableInputMeta targettii = new TableInputMeta();
		targettii.setDatabaseMeta(targetDatabase);
		targettii.setSQL("SELECT ITEM_ID,ITEM_ID2,SCORE FROM RECOMMEND_BASE_GR_ITEM_TABLE");
		StepMeta targetQuery = new StepMeta("target", targettii);
		transMeta.addStep(targetQuery);
		targetQuery.setLocation(150, 300);
		targetQuery.setDraw(true);
		...
		TableOutputMeta toi = new TableOutputMeta();
		toi.setDatabaseMeta(targetDatabase);
		toi.setTableName("RECOMMEND_BASE_GR_ITEM_TABLE");
		toi.setCommitSize(100);
		toi.setTruncateTable(false);
		toi.setSpecifyFields(true);
		toi.setFieldDatabase(allColumns);
		...
		um.setDatabaseMeta(targetDatabase);
		um.setUseBatchUpdate(true);
		um.setTableName("RECOMMEND_BASE_GR_ITEM_TABLE");
		um.setCommitSize("100");
		um.setKeyLookup(pkColumns);
		um.setKeyStream(pkColumns);
		um.setKeyCondition(conditions);
		...
		DeleteMeta dm = new DeleteMeta();
		dm.setDatabaseMeta(targetDatabase);
		dm.setTableName("RECOMMEND_BASE_GR_ITEM_TABLE");
		dm.setCommitSize("100");
		dm.setKeyCondition(conditions);
		dm.setKeyLookup(pkColumns);
		dm.setKeyStream2(new String[2]);
	...
	public KettleJobEntireDefine createJob() throws KettleException {
		...
		TransMeta firstTransMeta = createFirstTrans();
		TransMeta secondTransMeta = createSecondTrans();
		kettleJobEntireDefine.getDependentTrans().add(firstTransMeta);
		kettleJobEntireDefine.getDependentTrans().add(secondTransMeta);
		...
		// 第一个执行任务
		JobEntryTrans firstTran = new JobEntryTrans(firstTransMeta.getName());
		firstTran.setTransObjectId(firstTransMeta.getObjectId());
		firstTran.setWaitingToFinish(true);
		// 当前目录,即job的同级目录
		firstTran.setDirectory("${Internal.Entry.Current.Directory}");
		firstTran.setTransname(firstTransMeta.getName());
		JobEntryCopy firstExcuter = new JobEntryCopy(firstTran);
		firstExcuter.setLocation(300, 100);
		firstExcuter.setDrawn(true);
		firstExcuter.setDescription("MAINJOB");
		mainJob.addJobEntry(firstExcuter);
		// 连接
		JobHopMeta firsthop = new JobHopMeta(start, firstExcuter);
		mainJob.addJobHop(firsthop);
		kettleJobEntireDefine.setMainJob(mainJob);
		// 第二个执行任务
		JobEntryTrans secondTrans = new JobEntryTrans(secondTransMeta.getName());
		secondTrans.setTransObjectId(secondTransMeta.getObjectId());
		secondTrans.setWaitingToFinish(true);
		// 当前目录,即job的同级目录
		secondTrans.setDirectory("${Internal.Entry.Current.Directory}");
		secondTrans.setTransname(secondTransMeta.getName());
		JobEntryCopy secondExcuter = new JobEntryCopy(secondTrans);
		secondExcuter.setLocation(300, 300);
		secondExcuter.setDrawn(true);
		secondExcuter.setDescription("SECONDJOB");
		mainJob.addJobEntry(secondExcuter);
		// 连接
		JobHopMeta secondHop = new JobHopMeta(firstExcuter, secondExcuter);
		mainJob.addJobHop(secondHop);
		kettleJobEntireDefine.setMainJob(mainJob);
