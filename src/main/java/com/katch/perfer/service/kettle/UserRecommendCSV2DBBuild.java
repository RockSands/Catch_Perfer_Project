...
public class UserRecommendCSV2DBBuild {
	...
	private TransMeta createTrans() throws KettleException {
		...
		String[] pkColumns = new String[] {"USER_ID","ITEM_ID"};
		String[] allColumns = new String[] { "USER_ID", "ITEM_ID", "SCORE" };
		/*
		 * 条件
		 */
		...
		ci.setFilename(recommendPropeties.getUserRecommendFileName());
		...
		targettii.setSQL("SELECT USER_ID,ITEM_ID,SCORE FROM RECOMMEND_BASE_USER_TABLE");
		...
		um.setTableName("RECOMMEND_BASE_USER_TABLE");
		...
		dm.setTableName("RECOMMEND_BASE_USER_TABLE");
